...
单文件、浏览器直开、零依赖。它叫「可交互蓝图」——用项目真实设计令牌渲染、关键行为能上手点,既是设计文档也是可操作原型。蓝图里那块可点的界面本体叫**交互稿**,跟 AI 沟通时直接说"交互稿"即可(如"把交互稿里的侧栏改窄")。

**部署建议:** 见 [DEPLOYMENT.md](DEPLOYMENT.md)(部署 / 私有化建议,运维参考)。
...
| `index.js` | 登记清单(`var BLUEPRINTS = [...]`,支持文件夹分组;另有 `ADAPT_PRESETS` 常用画布尺寸参考(纯注册表,生成蓝图时在交互稿下方标注「画布 W×H」);内置一条默认示例,由 AI 生成后登记) |
